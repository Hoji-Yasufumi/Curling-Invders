...
void loop() 
{
    ...
    //SerialBT.println((int)head_dir);
    
    /*
    byte outinfo[8];
    
    outinfo[0] = 's';
    outinfo[1] = (int16_t)(((head_dir+180)/2) * 100) >> 8;
    outinfo[2] = (int16_t)(((head_dir+180)/2) * 100) & 0xFF;
    outinfo[3] = (int16_t)(dev.data.x * -1 * 100) >> 8;
    outinfo[4] = (int16_t)(dev.data.x * -1 * 100) & 0xFF;
    outinfo[5] = (int16_t)(dev.data.y * -1 * 100) >> 8;
    outinfo[6] = (int16_t)(dev.data.y * -1 * 100) & 0xFF;
    outinfo[7] = 'e';
    
    SerialBT.write(outinfo,8);
    */
    
    
    //SerialBT.write((int)((head_dir+180)/2));  //シリアル通信で8bit内かつ正の整数にするための補正
    SerialBT.print((int)head_dir+180);
    ...
    sprintf(text_string, "HEAD Angle: %d", (int)head_dir+180);
    img.drawString(text_string, 10, 110, 4);
    sprintf(text_string, "HEAD Angle Re: %d", (int)((head_dir+180)/2));
    img.drawString(text_string, 10, 140, 4);
    img.drawCentreString("Press BtnA enter calibrate", 160, 180, 4);
    img.pushSprite(0, 0);
